...
#include "config.h"
static int furiosa_crypto_rand(void *rng_state, unsigned char *output,
			       size_t len)
{
	size_t use_len;
	static int rnd = 7;
...
void sign_pub_key()
{
	...
	printf("sha256 start %s\n",
	       mbedtls_sha256_starts_ret(&sha_ctx,
					 0) == 0 ? "SUCCESS" : "FAIL");
	printf("sha256 update %s\n",
	       mbedtls_sha256_update_ret(&sha_ctx, data_buffer,
					 PUB_KEY_SIZE_IN_BYTES * 2) ==
	       0 ? "SUCCESS" : "FAIL");
	printf("sha256 finish %s\n",
	       mbedtls_sha256_finish_ret(&sha_ctx,
					 m) == 0 ? "SUCCESS" : "FAIL");
	printf("Sign %s\n",
	       mbedtls_ecdsa_sign_det(&eck->grp, &r, &s, &eck->d, m,
				      PUB_KEY_SIZE_IN_BYTES,
				      MBEDTLS_MD_SHA256) ==
	       0 ? "SUCCESS" : "FAIL");

	fd = open(ECDHKEY_PUB_SIGNED_MBED_FILE, O_CREAT | O_RDWR | O_TRUNC,
		  0644);
	if (fd < 0) {
		perror("open");
		return;
...
int main()
{
	...
	ret =
	    mbedtls_ecp_gen_key((mbedtls_ecp_group_id) MBEDTLS_ECP_DP_SECP256R1,
				ecp, furiosa_crypto_rand, NULL);
