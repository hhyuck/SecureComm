...
void generate_ec_key_pair(mbedtls_pk_context * key)
{
	...
	mbedtls_pk_init(key);
	ret =
	    mbedtls_pk_setup(key, mbedtls_pk_info_from_type((mbedtls_pk_type_t)
							    MBEDTLS_PK_ECKEY));
	ret =
	    mbedtls_ecp_gen_key((mbedtls_ecp_group_id) MBEDTLS_ECP_DP_SECP256K1,
				mbedtls_pk_ec(*key), furiosa_crypto_rand, NULL);
}
...
int main()
{
	...
	mbedtls_ecp_mul(&ecp_A->grp, &shared_key_A, &ecp_A->d, &ecp_B->Q, NULL,
			NULL);
	...
	mbedtls_ecp_mul(&ecp_B->grp, &shared_key_B, &ecp_B->d, &ecp_A->Q, NULL,
			NULL);
