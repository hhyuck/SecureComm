...
int main()
{
	...
	ret =
	    mbedtls_ecp_group_load(&eck->grp,
	...
	ret =
	    mbedtls_mpi_grow(&eck->d,
			     PRIV_KEY_SIZE_IN_BYTES /
			     sizeof (mbedtls_mpi_uint));
	ret =
	    mbedtls_mpi_grow(&eck->Q.X,
			     PUB_KEY_SIZE_IN_BYTES / sizeof (mbedtls_mpi_uint));
	ret =
	    mbedtls_mpi_grow(&eck->Q.Y,
			     PUB_KEY_SIZE_IN_BYTES / sizeof (mbedtls_mpi_uint));
	...
	ret =
	    mbedtls_mpi_grow(&ecp.X,
			     PUB_KEY_SIZE_IN_BYTES / sizeof (mbedtls_mpi_uint));
	ret =
	    mbedtls_mpi_grow(&ecp.Y,
			     PUB_KEY_SIZE_IN_BYTES / sizeof (mbedtls_mpi_uint));
	...
	printf("LINE %d %d\n", __LINE__,
	       mbedtls_ecp_mul(&eck->grp, &shared_key, &eck->d, &ecp, NULL,
			       NULL));
	...
	mbedtls_mpi_write_file("premaster secret : ", &premaster_secret, 16,
			       stdout);
	...
	mbedtls_md_hmac(mbedtls_md_info_from_type(MBEDTLS_MD_SHA256),
			(unsigned char *)premaster_secret.p,
			mbedtls_mpi_size(&premaster_secret),
			(unsigned char *)&MASTER_SECRET_SEED[0],
			strlen(MASTER_SECRET_SEED), master_secret);
	...
	mbedtls_md_hmac(mbedtls_md_info_from_type(MBEDTLS_MD_SHA256),
			(unsigned char *)premaster_secret.p,
			mbedtls_mpi_size(&premaster_secret),
			(unsigned char *)seed, strlen((char *)seed),
			master_secret + 32);
	...
	mbedtls_md_hmac(mbedtls_md_info_from_type(MBEDTLS_MD_SHA256),
			...
			(unsigned char *)&KEY_EXPANSION_SEED[0],
			strlen(KEY_EXPANSION_SEED), keys);
